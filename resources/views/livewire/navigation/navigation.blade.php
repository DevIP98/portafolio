            ...
            <nav class="relative flex items-center justify-between sm:h-10 lg:justify-start" aria-label="Global">
                ...
                <div class="hidden md:block md:ml-10 md:pr-4 md:space-x-8 text-lg">
                    @foreach ($items as $item)
                        <a href="{{ $item->link }}"
                            class="font-medium text-gray-200 hover:text-red-300">{{ $item->label }}
                        </a>
                    @endforeach
                </div>
        ...
        <div x-show="open" x-transition:enter="duration-150 ease-out" x-transition:enter-start="opacity-0 scale-95"
            ...
            <div class="rounded-lg shadow-md bg-white ring-1 ring-black ring-opacity-5 overflow-hidden">
                ...
                <div class="px-2 pt-2 pb-3 space-y-1">
                    @foreach ($items as $item)
                        <a href="{{ $item->link }}"
                            class="font-medium block px-3 py-2 rounded-md text-base text-gray-700 hover:text-gray-900 hover:bg-gray-50">{{ $item->label }}
                        </a>
                    @endforeach
                    <!-- Aqui botones edit y add -->
                </div>
